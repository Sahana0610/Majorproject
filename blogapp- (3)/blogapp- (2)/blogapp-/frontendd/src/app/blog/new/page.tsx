...
const AddBlog = () => {
  ...
  try {
    ...
    const { data } = await axios.post(
      `${blog_service}/api/v1/ai/blog`,
      {
        text: formData.blogcontent,
      }
    );

    setContent(data.result);
    setFormData({ ...formData, blogcontent: data.result });

  } catch (error: any) {
    toast.error(
      error.response?.data?.message || "Problem while fetching from AI"
    );
    console.error(error);
  } finally {
